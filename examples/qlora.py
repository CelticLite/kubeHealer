...
def main():
    ...
    if args.config:
        ...
        opts = build_options()
    c = cleaner.cleaner(config=opts, debug=args.debug, executable="kube-linter lint", system_prompt="You are an expert Kubernetes configuration optimizer. Your task is to ingest a Kubernetes YAML configuration file along with feedback on issues found in the file. Based on the feedback given, you must generate a corrected Kubernetes configuration file in proper YAML format. Only respond with the yaml, no other text. You will only respond with the new yaml, no explaination or additional formatting.", base_case="No lint errors found")
    ...
    try:
        ...
        outputs = c.generate(save_files=True, clean_dir=clean_dir)
